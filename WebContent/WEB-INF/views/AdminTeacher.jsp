...
<h1>UserItems</h1>
	...
	<div id="myTable">
	<div id="btnCourse">
	  <a href="${pageContext.request.contextPath}/teachercreate" type="submit" class="btn btn-outline-success btn-lg">Create</a>
	  </div>
	<div class="row">
		...
		<thead>
         ...
         <c:set var="currentTeachers" value="${teacherAdminDTO.teachers}"/>
			<c:forEach var="row" items="${currentTeachers}">
				<tr>
					<td>${row.getId()}</td>
					<td>${row.getFirst_name()}</td>
					<td>${row.getLast_name()}</td>
					<td>${row.getExpierence()}</td>
					<td><a class="btn btn-outline-light" href="${pageContext.request.contextPath}/teacheredit?idTeacher=${row.getId()}">Edit</a></td>
					<td><a class="btn btn-outline-danger" href="" onclick="checkDeleteItem('${pageContext.request.contextPath}/teacherdelete?idTeacherDelete=${row.getId()}')">Delete</a></td>
				</tr>
			</c:forEach>	
		</table>
		</div>
