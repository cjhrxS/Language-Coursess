...
public class TeacherAdminController extends HttpServlet {
	...
	private static final long serialVersionUID = -5085391542840817136L;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!isActiveSession) {
			...
			TeacherAdminDTO teacherAdminDTO = teacherService.getAdminUser();
			request.setAttribute("teacherAdminDTO", teacherAdminDTO);
		    request.setAttribute("countTeachers", 
		    		String.valueOf(teacherAdminDTO.getTeachers().size()));
			...
			getServletConfig()
				.getServletContext()
				.getRequestDispatcher(ViewUrls.ADMIN_TEACHER_JSP.toString())
				.forward(request, response);
		}
	}
